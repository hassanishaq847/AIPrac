...
class OverlayView(context: Context?, attrs: AttributeSet?) :
    ...
    private val TAG: String = OverlayView::class.java.name
    ...
    fun setResults(
        ...
        for (i in pixels.indices) {
            ...
            val color: Int = if (index.toInt() == 1) {
                ImageSegmenterHelper.labelColors[ImageSegmenterHelper.labelColors.size - 1].toAlphaColor()
            } else {
                ImageSegmenterHelper.labelColors[index.toInt()].toAlphaColor()
            }
            pixels[i] = color

            // just converting and showing colors in hex
//            val labelColorsHex = ImageSegmenterHelper.labelColors.map { colorInt ->
//                String.format("#%06X", 0xFFFFFF and colorInt)
//            }
//            labelColorsHex.forEach { hexString ->
//                Log.d(TAG, "listColors: $hexString")
//            }
        }
